...
``philo.py``, ``josephus.py``, ``bare_authors.py``, and in time
``bekker.py``, ``diels_kranz.py``, unit systems); the engine that ties
them together lives in ``base.py``.
"""

from redaction.elocution.bare_authors import BARE_AUTHORS, bare_authors_system
from redaction.elocution.base import Elocutor, System, bare_author_system, mechanical_locator
...
def default_systems() -> tuple[System, ...]:
    ...
    Philo, Josephus, and the bare-author list don't currently collide
    with the others or each other, so their position is arbitrary. Grows
    as Bekker/Diels-Kranz systems land.
    """
    return (
        classical_system(),
        stephanus_system(),
